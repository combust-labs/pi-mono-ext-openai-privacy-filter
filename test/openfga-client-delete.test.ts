...
/**
 * Phase 4.2: OpenFGAClient.deleteTuples() Unit Tests
 *
 * Tests cover:
...
describe('OpenFGAClient.deleteTuples()', () => {
  ...
  it('hashes each tuple literal before deleting from OpenFGA', async () => {
    ...
    const tupleKeys = body.deletes.tuple_keys;
    assert.ok(Array.isArray(tupleKeys), 'deletes.tuple_keys should be an array');
  ...
  it('sends tuples under the deletes key in request body', async () => {
    fetchMock.mockResponse({ ok: true, status: 200, statusText: 'OK', body: {} });
    ...
    const body = JSON.parse(request!.options.body as string);
    // Must use "deletes" key, not "writes"
    assert.ok('deletes' in body, 'Request body should have deletes key');
    assert.ok(!('writes' in body), 'Request body should not have writes key');
    assert.ok(Array.isArray(body.deletes.tuple_keys), 'deletes.tuple_keys should be an array');
  });
  ...
  it('sends correct URL with store ID and /write endpoint', async () => {
    ...
    assert.ok(request!.url.includes('/write'), 'URL should include /write endpoint (delete uses same endpoint)');
  });
  ...
  it('throws on non-2xx response with status and body in error message', async () => {
    fetchMock.mockResponse({
      ...
      body: { message: 'Delete operation failed' }
    });

    await assert.rejects(
      async () => client.deleteTuples([
        { subject: 'test-model', relation: 'can_view', object: 'email' }
      ]),
      (err: Error) => {
        assert.ok(
        ...
        assert.ok(
          err.message.includes('Delete operation failed'),
